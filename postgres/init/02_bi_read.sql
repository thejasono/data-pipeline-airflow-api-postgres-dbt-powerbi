...
-- Goal: allow Power BI (or other BI tools) to read public_analytics, raw, and public
--       with SELECT privileges; deny public_staging.
...
-- 2a) Deny access to public_staging (defense-in-depth)
...
-- public
GRANT USAGE ON SCHEMA public TO bi_read;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO bi_read;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO bi_read;
...
-- (Optional but recommended if you use materialized views)
DO
$$
BEGIN
    -- These may fail on older PG if the schema has no matviews yet; wrap if desired.
    EXECUTE 'GRANT SELECT ON ALL MATERIALIZED VIEWS IN SCHEMA public TO bi_read';
    EXECUTE 'GRANT SELECT ON ALL MATERIALIZED VIEWS IN SCHEMA raw TO bi_read';
    EXECUTE 'GRANT SELECT ON ALL MATERIALIZED VIEWS IN SCHEMA public_analytics TO bi_read';
EXCEPTION WHEN undefined_table THEN
    -- Ignore if server version doesn't support ALL MATERIALIZED VIEWS or none exist.
    NULL;
END
$$;
...
-- public
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO bi_read;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO bi_read;
...
-- (Optional) For matviews created later, most teams GRANT per-object after creation.

-- 4) QoL: make name resolution predictable (analytics first is typical)
ALTER ROLE bi_read SET search_path TO public_analytics, raw, public;
